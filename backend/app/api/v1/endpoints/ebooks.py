...
def create_ebook(
    *,
    ...
    ebook = Ebook(
        title=ebook_in.title,
        description=ebook_in.description,
        price=ebook_in.price,
        author=ebook_in.author,
        image=ebook_in.image,
        file_url=ebook_in.file_url
    )
    db.add(ebook)
    db.commit()
    db.refresh(ebook)
...
def update_ebook(
    *,
    ...
    update_data = ebook_in.dict(exclude_unset=True)
